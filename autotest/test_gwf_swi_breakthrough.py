...
buoyancy gate -- is that freshwater does NOT show up in a cell before the
...
saltwater zone, which the buoyancy gate suppresses.

Without the gate, the front leaves a small freshwater tail one cell ahead
(Sf~0.04 while the overlying cell is ~0.46); with the gate the cell below the
front stays dry until the front cell fills. This test asserts the gated (correct)
behavior and would fail if the vertical flow were ungated. It also checks that the
front descends at the mass-balance rate and that both budgets conserve mass.
...
def _saturation(test):
    ...
    sf = []
    for t in times:
        zeta = -alphaf * hf.get_data(totim=t).flatten() \
...
def plot_output(idx, test):
    ...
    for j, ti in enumerate(snap):
        ...
        for k in range(nlay):
            ...
            ax.add_patch(Rectangle((0, cell_bot[k]), 1.0, zc - cell_bot[k],
                                   facecolor="red", edgecolor="k", lw=0.4))
            ax.add_patch(Rectangle((0, zc), 1.0, cell_top[k] - zc,
                                   facecolor="blue", edgecolor="k", lw=0.4))
...
def check_output(idx, test):
    ...
    for it, t in enumerate(times):
        for k in range(1, nlay):
            if sf[it, k] > wet and sf[it, k - 1] < full:
                ...
                print(f"  premature at t={t:.1f} layer {k + 1}: "
    ...
    front = np.array([cell_bot[np.where(s > 0.5)[0][-1]] if np.any(s > 0.5)
                      else top for s in sf])
