module LnfOcModule
  ...
  implicit none
  private
  public LnfOcType, oc_cr

  !> @ brief Output control for LNF
  !!
  !!  Concrete implementation of OutputControlType for the
  !!  LNF Model
  !<
  type, extends(OutputControlType) :: LnfOcType
  contains
    procedure :: oc_ar
  end type LnfOcType
...
contains
  ...
  !> @ brief Create LnfOcType
  !!
  !!  Create by allocating a new LnfOcType object and initializing
  !!  member variables.
  !!
  !<
  subroutine oc_cr(ocobj, name_model, input_mempath, inunit, iout)
    ! -- dummy
    type(LnfOcType), pointer :: ocobj !< LnfOcType object
  ...
  !> @ brief Allocate and read LnfOcType
  !!
  ...
  subroutine oc_ar(this, head, dis, dnodata)
    ...
    use LnfOcInputModule, only: LnfOcParamFoundType
    ! -- dummy
    class(LnfOcType) :: this !< LnfOcType object
    ...
    type(LnfOcParamFoundType) :: found
    !
