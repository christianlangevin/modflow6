...
\noindent where $S_{s}$ is specific storage, $S_{y}$ is specific yield, $\tilde K^{f}$ and $\tilde K^{s}$ are the freshwater and saltwater hydraulic conductivities, and $\dot W_{f}$ and $\dot W_{s}$ are volumetric source and sink rates. Saltwater hydraulic conductivity is related to the freshwater value through the density and viscosity ratios,
...
\noindent where $\mu_{f}$ and $\mu_{s}$ are the freshwater and saltwater dynamic viscosities. The package does not apply this adjustment internally; the hydraulic conductivity entered for the saltwater model of a two-fluid simulation should be consistent with equation~\ref{eqn:swi-Ks}. For seawater the density and viscosity ratios nearly cancel, and the adjusted value differs from the freshwater value by only a few percent. Equations~\ref{eqn:swi-gh}, \ref{eqn:swi-pdef}, and \ref{eqn:swi-pdes} contain the three unknowns $h^{f}$, $h^{s}$, and $\zeta$. In single-fluid mode, only equation~\ref{eqn:swi-pdef} is solved and $\zeta$ is obtained from equation~\ref{eqn:swi-gh}. In two-fluid mode, equations~\ref{eqn:swi-pdef} and \ref{eqn:swi-pdes} are solved simultaneously for $h^{f}$ and $h^{s}$.
...
\noindent where $S_{nm}^{s,u}$ is the upstream saltwater saturated cell fraction and the saltwater conductance $C_{nm}^{s}$ is computed from the hydraulic conductivity specified for the saltwater model, which should reflect the density and viscosity adjustment of equation~\ref{eqn:swi-Ks} so that $C_{nm}^{s}=C_{nm}^{f}\,(\rho_{s}\mu_{f})/(\mu_{s}\rho_{f})$. Equations~\ref{eqn:swi-fvf} and \ref{eqn:swi-fvs} therefore have the same form, with the saturated cell fraction and conductance appropriate for each fluid.
